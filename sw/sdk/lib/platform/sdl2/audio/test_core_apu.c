/** Simulated apu core
 * - runs apu_main same as pico, inject hardware abstractions 
 * - audio mixer
 * - usb hid simulation from sdl -> hid messages
 * - sd card image emulation
 */
#include "lib/components/apu_core/apu_core.h"
#include "platform/pico/hw/picocom_hw.h"
#include "platform/pico/apu/hw_apu_types.h"
#include <string.h>
#include <stdio.h>


// Globals
struct apu_t* g_apu = 0;


//
//
void test_core_apu()
{    
    // Create bus links
    static BusTx_t app_alnk_tx;
    static BusRx_t app_alnk_rx;    

    // Init apu -> app link  
    bus_rx_configure(&app_alnk_rx,
            APU_ALNK_RX_PIO,
            APU_ALNK_RX_SM,       
            APU_ALNK_RX_DATA_CNT,
            APU_ALNK_RX_D0_PIN,
            APU_ALNK_RX_ACK_PIN
        );
    //bus_tx_pulse_debug_input(&app_alnk_rx);
    bus_rx_init(&app_alnk_rx);

    bus_tx_configure(
        &app_alnk_tx,
        APU_ALNK_TX_PIO,
        APU_ALNK_TX_SM,
        APU_ALNK_TX_DATA_CNT,
        APU_ALNK_TX_D0_PIN,
        APU_ALNK_TX_ACK_PIN,
        APU_ALNK_TX_IRQ,
        ALNK_DIV
    );
    app_alnk_tx.name = "app_alnk_tx";
    bus_tx_init( &app_alnk_tx );
    
    struct apuMainLoopOptions_t mainOptions;
    struct apuInitOptions_t vdpOptions;
    g_apu = ( struct apu_t*)picocom_malloc(sizeof(struct apu_t));
    memset(g_apu, 0, sizeof(struct apu_t));
    memset(&mainOptions, 0, sizeof(mainOptions));
    memset(&vdpOptions, 0, sizeof(vdpOptions));

    vdpOptions.app_alnk_tx = &app_alnk_tx;
    vdpOptions.app_alnk_rx = &app_alnk_rx;
    
    // init vdp core
    int res;
    res = apu_init(g_apu, &vdpOptions);
    if(res != SDKErr_OK)
    {
        printf("apu_init failed %d\n", res);
        return;
    }

#ifdef PICOCOM_NATIVE_SIM
    // single threaded update
#else
    // run main loop
    apu_main(g_apu, &mainOptions);
#endif    
}

#ifdef PICOCOM_NATIVE_SIM    
void test_core_apu_full_update()
{    
    apu_update(g_apu, 0);
}

void test_core_apu_render_audio()
{
    struct apuMainLoopOptions_t options = {.audioRenderOnly=1};
    apu_update(g_apu, &options);
}

void test_service_audio_main(void* userData)
{
    apu_update(g_apu, 0);
}

void test_service_storage_main(void* userData)
{
    apu_update(g_apu, 0);
}


void test_service_input_main(void* userData)
{
    apu_update(g_apu, 0);
}

#else

void test_core_apu_update()
{
    // threaded
}


void test_service_audio_main(void* userData)
{
    // threaded
}

void test_service_storage_main(void* userData)
{
    // threaded
}


void test_service_input_main(void* userData)
{
    // threaded
}

#endif