...
{   
    // hack to lookup, will write index as array, requires quite some formatting and too fucked to care
    // NOTE: shouldbe faster in ram, not sure how might different per tile though
    static bool tooLazyToWriteTilemapAsStaticBufferInFlash = false;
    if(!tooLazyToWriteTilemapAsStaticBufferInFlash)
    {
        initTilemap();
        tooLazyToWriteTilemapAsStaticBufferInFlash = 1;
    }
